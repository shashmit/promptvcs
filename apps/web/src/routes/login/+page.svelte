<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    loading = true;
    try {
      const result = await authApi.signIn(email, password) as any;
      if (result?.user) {
...
<svelte:head><title>Sign in — PromptVCS</title></svelte:head>
...
<div class="min-h-screen flex items-center justify-center bg-surface-950 px-4 relative">
  ...
  <div class="w-full max-w-md relative z-10">
    <div class="text-center mb-8">
      ...
      <p class="text-zinc-500 mt-2">Sign in to your PromptVCS account</p>
    </div>
    ...
    <form on:submit|preventDefault={handleSubmit} class="card space-y-5">
      ...
      <div>
        ...
        <input id="password" type="password" bind:value={password} class="input" placeholder="••••••••" required />
      </div>
    ...
    <p class="text-center text-zinc-600 text-sm mt-6">
      Don't have an account? <a href="/register" class="text-brand-300 hover:text-brand-200 transition-colors">Sign up</a>
    </p>
  </div>
</div>
